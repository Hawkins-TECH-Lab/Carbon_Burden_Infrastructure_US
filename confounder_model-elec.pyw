...
elec_df['Decile_Density'] = pd.qcut(elec_df['treat_density'], 10, labels=False)
elec_df['Decile_Diversity'] = pd.qcut(elec_df['treat_diversity'], 10, labels=False) 
...
for i in range(10):
    temp_df = elec_df.loc[elec_df['Decile_Density']==i] 
    X = temp_df.loc[:, var_list]
    y_res = temp_df["res_tc"]
    y_tot = temp_df["res_tc"]  + temp_df["com_tc"] + temp_df["ind_tc"]

    vi_results = make_confounder_model(X, y_res, samples=2000)
    temp_importance_df = get_splits(vi_results, var_list, "res", "dens", i)
    importance_df = pd.concat([importance_df, temp_importance_df], ignore_index=True)
    
    vi_results = make_confounder_model(X, y_tot, samples=2000)
    temp_importance_df = get_splits(vi_results, var_list, "tot", "dens", i)
    importance_df = pd.concat([importance_df, temp_importance_df], ignore_index=True)
    
for i in range(10):
    temp_df = elec_df.loc[elec_df['Decile_Diversity']==i] 
    X = temp_df.loc[:, var_list]
    y_res = temp_df["res_tc"]
    y_tot = temp_df["res_tc"]  + temp_df["com_tc"] + temp_df["ind_tc"]

    vi_results = make_confounder_model(X, y_res, samples=2000)
    temp_importance_df = get_splits(vi_results, var_list, "res", "div", i)
    importance_df = pd.concat([importance_df, temp_importance_df], ignore_index=True)
    
    vi_results = make_confounder_model(X, y_tot, samples=2000)
    temp_importance_df = get_splits(vi_results, var_list, "tot", "div", i)
    importance_df = pd.concat([importance_df, temp_importance_df], ignore_index=True)
    
importance_df.to_csv("/work/hawkins_lab/vulcan/results/importance_electricity.csv", index=False)